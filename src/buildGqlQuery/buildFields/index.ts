...
export const buildFields =
    ...
    ({
        ...
        let { fields } = supabaseFieldsForObject({
        ...
        if (nodeIdField && !resourceFields.find(f => f.name === 'nodeId'))
            resourceFields.push(nodeIdField);
        ...
        return resourceFields.reduce((acc: any, field: IntrospectionField) => {
            ...
            if (linkedResourceObject) {
                let { object, fields: linkedResourceFields } =
                    supabaseFieldsForObject({
                        introspectionResults,
                        resourceObject: linkedResourceObject,
                        fields: linkedResourceObject.type
            ...
            if (!gqlSelectionSet) {
                ...
                const { object, fields: linkedTypeFields } =
                    supabaseFieldsForObject({
                        introspectionResults,
