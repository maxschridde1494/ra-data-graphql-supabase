...
import { UPDATE, UPDATE_MANY } from 'ra-core';
import { IntrospectionResult, IntrospectedResource } from 'ra-data-graphql';

export const buildCreateUpdateVariables =
    ...
    (
        ...
        let dataType: {
            key: 'objects' | 'set';
            type: TypeKind.LIST | TypeKind.OBJECT;
        };
        ...
        if (raFetchMethod === UPDATE || raFetchMethod === UPDATE_MANY) {
            ...
            const updateInputTypeName = (
                queryType.args.find(a => a.name === dataType.key).type as any
            ).ofType.name;
            inputType = introspectionResults.types.find(
                t => t.name === updateInputTypeName
            ) as IntrospectionInputObjectType;

            variables.filter = {
                id: raFetchMethod === UPDATE ? { eq: id } : { in: ids },
            };
            variables.atMost = raFetchMethod === UPDATE ? 1 : ids.length;
        } else {
            dataType = { key: 'objects', type: TypeKind.LIST };

            const instertInputTypeName = (
                queryType.args.find(a => a.name === dataType.key).type as any
            ).ofType.ofType.ofType.name; // list type so doubly nested ofTypes
            inputType = introspectionResults.types.find(
                t => t.name === instertInputTypeName
            ) as IntrospectionInputObjectType;
        }
        ...
        const sanitizedData = Object.keys(data).reduce((acc, key) => {
            if (!inputType.inputFields.find(f => f.name === key)) {
                console.info(
                    `Field ${key} is not available on type ${inputType.name}`
                );
                return acc;
            }
        ...
        if (dataType.type === TypeKind.LIST)
            variables[dataType.key] = [variables[dataType.key]];
